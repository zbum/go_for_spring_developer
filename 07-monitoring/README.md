...
```
github.com/zbum/scouter-agent-golang v0.20.0-alpha.3
````
...
* handler 함수의 경우, 다음의 코드를 사용할 수 있습니다.
```go
func Proxy(f func(http.ResponseWriter, *http.Request)) func(http.ResponseWriter, *http.Request) {
    ...
    step := strace.StartCustomMethod(ctx, methodName)
...
* 설정파일의 예
```
obj_name=prototype-go-w4
obj_host_name=k8s-prototype
...
* K8s 의 경우 env 로 설정할 수 있습니다.

```yaml
    spec:
      containers:
        - name: prototype-go
          image: registry.op.internal.dooray.io/dooray/prototype-go:${VERSION}
          env:
            - name: SCOUTER_CONFIG
              value: /scouter/prototype-go.conf
```


## 결과
* 다음과 같이 메소드 프로파일이 적용되었는지 확인합니다.
  ![Inline-image-2023-07-25 12.41.22.234.png](/wikis/1567969277598670267/files/3589925618879198859)

* api 호출 프로파일을 적용하면 다음과 같이 프로파일에 표시가 됩니다.
  ![Inline-image-2023-07-25 12.40.36.528.png](/wikis/1567969277598670267/files/3589925236295953931)
* db 호출 프로파일을 적용하면 다음과 같이 프로파일에 실행한 SQL 이 표시가 됩니다.
  ![Inline-image-2023-07-25 12.29.37.266.png](/wikis/1567969277598670267/files/3589919706197756662)
